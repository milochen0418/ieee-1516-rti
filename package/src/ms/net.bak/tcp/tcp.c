...
extern struct queue_item_t sendq[SENDQ_MAX];
extern struct queue_item_t recvq[RECVQ_MAX];
extern struct queue_item_t socketq;
extern pthread_mutex_t sendq_mutex;
extern pthread_mutex_t recvq_mutex;
...
static struct service_list_t *service_list = NULL;
...
void process_send_messages()
{
	...
	list_for_each(l, &sendq[SENDQ_INPROGRESS].list)
	{
		...
		if (data->iov_len_done != 0){
			io_vector[count].iov_base = data->buffer_list[data->iov_idx];
		...
		ret = sockio_nbvector(network_key->socket, io_vector, count, TCP_SEND);
		if (ret < 0){
		...
		for (i=data->iov_idx; i<data->list_count; i++){
			...
			if (total_size > data->complete_size){
				data->iov_idx = i;
				data->iov_len_done = total_size - data->complete_size;
				break;
			}
		...
		for (i=0; i<socket_count; i++){
			if (recv_pollfd[i].revents && (recv_pollfd[i].revents & POLLIN)){
				...
				if (item != NULL){
					...
					ret = sockio_nbrecv(recv_pollfd[i].fd, &((char *)data->buffer)[data->complete_size], 
							data->size - data->complete_size);
					
					if (ret < 0){
						/* TODO */
						error("%s: TODO: error handle\n", __func__);
					}
				...
				/* 
				 * No any previous recveive request is pending.
				 * And a new receive request for this socket number arrives.
				 */
				ret = sockio_nbpeek(recv_pollfd[i].fd, &tcp_header, TCP_HEADER_SIZE);
				if (ret < header_size){
				...
				if (tcp_header.magic_nr != COMM_MAGIC_NR){
					/* TODO */
					error("magic number not the same\n");
				}
				...
				ret = sockio_brecv(recv_pollfd[i].fd, &tcp_header, TCP_HEADER_SIZE);
				...
				ret = sockio_nbrecv(recv_pollfd[i].fd, data->buffer, data->size);
				if (ret < 0){
					/* TODO */
					error("%s: TODO: error handle\n", __func__);
				}
...
int setup_tcp_module(void)
{
	...
	service_list = Malloc(sizeof(struct service_list_t));
	memset(&service_list->server, 0, sizeof(struct server_addr_t));
...
static int init_server(struct server_addr_t *addr){
	...
	sockio_set_sockopt(sock, SO_REUSEADDR, 1);
	
	/* bind it to the appropriate port */
	if ((sockio_bind_sock(sock, addr->hostname, addr->port)) == -1){
...
int TCP_Initialize(char *hostname, int port, int init_flags)
{
	...
	if (init_flags & INIT_VSERVER){
		if (service_list == NULL)
			error("server TCP/IP module is not initialized\n");

		if (port < 0)
			error("Invalid port number\n");
		
		l = &service_list->list;
		for (;l->next != (list_t *)service_list; l=l->next){
			service = (struct service_list_t *)l->next;
			if (service->server.port == port){
		...
		addr->hostname = NULL;	/* NOT used for TCP/IP module */
		addr->port = port;
...
int TCP_Connect(struct host_addr_t *addr)
{
	...
	if ((addr->socket = sockio_new_sock()) < 0)
	...
	if (sockio_set_tcpopt(addr->socket, TCP_NODELAY, 1) < 0)
		error("failed to set TCP_NODELAY option.\n");

	ret = sockio_connect_sock(addr->socket, addr->hostname, addr->port);
	if (ret < 0){
		if (errno == EINPROGRESS)
			return 0;
		else
			error("sockio_connect_sock: %s\n", strerror(errno));
	}
 ...
 * Accept a connection
 */
int try_accept_connection(int server_sock)
{
	int sock;
	...
	if (sockio_set_tcpopt(sock, TCP_NODELAY, 1) < 0){
		error("%s: internel error\n", __func__);
	}
...
int TCP_Isend(int dest, void *buffer, comm_size_t size, comm_tag_t tag, Request *request)
{
	...
	pthread_mutex_lock(&sendq_mutex);
	item = search_queue(&sendq[SENDQ_INPROGRESS], network_key, sizeof(struct network_key_t));
	...
	ret = sockio_bsend(dest, &tcp_header, TCP_HEADER_SIZE);
	if (ret != TCP_HEADER_SIZE)
		error("internal error\n");
	
	/* The data payload are sent by nonblocking send */
	ret = sockio_nbsend(dest, buffer, size);
		...
		int list_count, comm_tag_t tag, Request *request)
{
	...
	ret = sockio_nbvector(dest, io_vector, list_count, TCP_SEND);
...
int check_new_connection()
{
	...
	l = service_list->list.next;
	while (l != (list_t *)service_list){
		struct service_list_t *service = (struct service_list_t *)l;
		
		if (try_accept_connection(service->server.socket) > 0)
			return 1;
