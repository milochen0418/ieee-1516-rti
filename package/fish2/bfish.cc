	...
	try{
		...
		while(1){
			for(i=0;i<total;i++){
				...
				if(local[i].age>=10&&rand()%3==0){
					eatable=0;
					for(j=0;j<foodtotal;j++){
						if(abs(food[j].x-local[i].x)<100&&abs(food[j].y-local[i].y)<100){
							eatable=1;
							break;
						}
						//if(abs(food[j].y-local[i].y)<10){
						//	eatable=1;
						//	break;
						//}
					}
...
void addfish(int i){
	...
	local[total].x=local[i].x;
	local[total].y=local[i].y;
	local[total].age=0;
	total++;
	FIVM.clear();
	pthread_mutex_unlock (&totalmux);
